...
User模块是Euler Copilot框架中的核心用户管理模块，负责处理用户信息的CRUD操作、用户LLM配置管理、用户标签管理等功能。该模块采用分层架构设计，包含数据模型层、服务层和路由层。
...
1. **模型验证**: 验证用户选择的LLM模型是否存在
2. **数据更新**: 更新用户表中的functionLLM和embeddingLLM字段
3. **向量化处理**: 当embedding模型变化时，自动触发向量化过程
