...
class UserExporter:
    ...
    @staticmethod
    def export_user_info_to_xlsx(tmp_out_dir: Path | str, user_sub: str) -> None:
        """
        将用户信息导出到xlsx文件中。

        Args:
            tmp_out_dir (Path | str): 临时输出目录的路径
            user_sub (str): 用户的唯一标识符

        Returns:
            None

        Raises:
            ValueError: 当用户信息获取失败时抛出

        """
        info = UserManager.get_userinfo_by_user_sub(user_sub)
        fname = f"user_info_{user_sub}.xlsx"
        fname = re.sub(r'[<>:"/\\|?*]', "_", fname)
        fname = fname.replace(" ", "_")
        fpath = Path(tmp_out_dir) / fname
        UserExporter.save_user_info_to_xlsx(fpath, info)
