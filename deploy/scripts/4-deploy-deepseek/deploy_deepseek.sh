...
check_local_model() {
    echo -e "${BLUE}步骤2/5：检查本地模型...${NC}"
    if [[ -f "${MODEL_DIR}/${MODEL_FILE}" ]]; then
        echo -e "${GREEN}检测到本地模型文件，使用离线安装${NC}"
    else
        echo -e "${RED}错误：找不到本地模型文件 ${MODEL_DIR}/${MODEL_FILE}${NC}"
        echo -e "${YELLOW}请将模型文件放置在 ${MODEL_DIR} 或连接网络后重试${NC}"
        exit 1
    fi
}
