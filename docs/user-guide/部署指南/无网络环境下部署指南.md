...
1. 部署前需准备好所有资源
2. 需输入 Authhub 和 openEuler Intelligence 域名（默认使用 `authhub.eulercopilot.local`, `www.eulercopilot.local`）
...
1. **访问知识库管理系统**：
   - 登录本地知识库管理页面（账号：`admin`，密码：`123456`)
   - 参考[本地资产库构建指南](./本地资产库构建指南.md)
